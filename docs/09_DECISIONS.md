...
- **Végleges megerősítés (2026-08-23, első kör)**: a felhasználó egy cPanel-ben generált SSH-kulcspárral (`id_rsa`, jelszóval védett) közvetlen kapcsolódási tesztet végeztünk (`ssh szablac@185.75.192.93`). A hitelesítés **sikeres** volt (kulcs + felhasználónév helyes), de a szerver egyértelmű választ adott: *"Shell access is not enabled on your account! If you need shell access please contact support."* — tehát ez nem technikai hiba vagy hiányzó beállítás, hanem a szolgáltató tudatos korlátozása a csomagon.
- **FELÜLBÍRÁLVA (2026-08-23, ugyanaznap, második/harmadik kör)**: a felhasználó jelezte a support felé fordulást, és a szolgáltató **engedélyezte a shell-hozzáférést**. Megismételt kapcsolódási teszt (`ssh szablac@185.75.192.93`) ezúttal **valódi shell-hozzáférést** adott (`whoami` → `szablac`, `hostname` → `c3.elin.hu`, kezdőkönyvtár `/home/szablac`). **Ez a jelen ADR alapfeltevését megváltoztatja** — innentől van tényleges shell-hozzáférés, ami megnyitja az utat a manuális lépések (kézi phpMyAdmin-migráció, cPanel GUI-kattintgatás a deploy-nál) automatizálására. **Ez még nem történt meg** — a git-deploy módszer (HTTPS, publikus repó) és a kézi migrációs munkamód egyelőre változatlanul érvényben marad, amíg a felhasználóval közösen át nem gondoljuk és jóvá nem hagyjuk az új munkafolyamatot (pl. SSH-alapú deploy, `mysql` CLI migrációkhoz). Az SSH-kulcs jelenleg `~/.ssh/id_rsa_qwer`-ben van (a fejlesztői gépen, nem a repóban), jelszóval védett.
...
### ADR-016 – SSH-alapú deploy-szkript és `.my.cnf` a migrációkhoz (fokozatos átállás)

- Állapot: **ELFOGADOTT**
- Dátum: 2026-08-23
- Döntés: az ADR-006-ban dokumentált, időközben engedélyezett shell-hozzáférésre építve két segédeszköz készült: (1) `scripts/deploy.sh` (verziókövetve, a repóban) — a szerveren futtatva egy paranccsal elvégzi a `git pull`-t, feltételesen (csak ha a `package-lock.json` változott) az `npm install`-t, majd a Passenger-újraindítást (`touch tmp/restart.txt`); leáll, ha a szerveren el nem mentett, trackelt módosítást talál. (2) `~/.my.cnf` a szerveren (repón kívül, `chmod 600`) — a `mysql` CLI ettől kezdve jelszó nélkül hitelesít, migrációk futtatásához: `mysql < db/migrations/0XX_nev.sql`.
- Indok: a felhasználó kifejezetten **fokozatos** átállást kért a kézi cPanel/phpMyAdmin munkamódról — ez az első lépés, nem cseréli le azonnal a jelenlegi módszert, csak elérhetővé teszi gyorsabb alternatívaként.
- **Tudatosan NEM automatizált**: a migráció futtatása a deploy-szkriptből — séma-módosítás mindig külön átgondolást érdemel (`CLAUDE.md` 4. szakasz), nem szabad "csendben" belecsempészni egy rutin kód-deploy-ba.
- **Biztonsági megjegyzés**: a `.htaccess` fájl SSH-s megtekintése közben egyszer véletlenül megjelent az éles DB-jelszó a beszélgetés kimenetében (a cPanel oda teszi a Node-alkalmazás env-változóit). Nem került git alá, nem lett újra megjelenítve, és a `.my.cnf` bevezetésével a jövőben migrációkhoz sem kell begépelni/kiírni. A felhasználó mérlegelheti a jelszó cseréjét a cPanel felületén, ha szeretné — ez nem sürgős, mivel a kiszivárgás a saját, helyi munkamenetén belül történt, nem harmadik félhez.
- **Ismert, tudatosan vállalt korlát**: ha maga a `deploy.sh` változik egy commitban, az éppen futó szkript a régi verzióval fejeződik be (bash már beolvasta a fájlt futtatáskor) — csak a *következő* futtatás használja az új verziót.
- Ellenőrzés: `bash -n scripts/deploy.sh` szintaktikailag hibátlan; a `.my.cnf` létrehozása után `mysql -e "SELECT 1"` jelszó nélkül sikeresen lefutott a szerveren.

### ADR-017 – AdSense hirdetéskód + GDPR consent-üzenet (Google Funding Choices)

- Állapot: **ELFOGADOTT**
- Dátum: 2026-08-23
- Döntés: az AdSense fiók-felülvizsgálat "beavatkozást igényel" jelzésére (hiányzó hirdetéskód) beépítettük az `adsbygoogle.js` szkriptet (`client=ca-pub-2290062414680227`) mind a 18 HU/EN oldal `<head>`-jébe. Emellett a Google saját, beépített consent-kezelő eszközével ("Adatvédelem és üzenetek" → "Európai szabályozások", Funding Choices) létrehoztunk és közzétettünk egy GDPR-üzenetet EGT/UK/Svájc látogatóknak, HU+EN nyelven.
- Indok: a `sutik.html`/`en/cookies.html` már korábban (ADR-007 óta) ígérte, hogy "amint az AdSense élesedik, sütibeleegyezés-üzenet jelenik meg" — ezt a hirdetéskód beillesztésével egyidejűleg, nem utólag kellett megoldani, hogy ne legyen olyan időszak, amikor élő hirdetési sütik futnak consent nélkül EGT-s látogatóknak.
- **Miért Google saját CMP-je, nem egyedi banner**: a Google Funding Choices IAB TCF-tanúsított, ingyenes, kód nélkül (fiók-szintű konfigurációval) beállítható, és automatikusan együttműködik az AdSense-szel — egyedi consent-banner építése feleslegesen duplikálná ezt.
- **Konkrét beállítások, amiket végigbeszéltünk**: „Nem járulok hozzá" (elutasítás) gomb bekapcsolva, „Bezárás (nincs beleegyezés)" bekapcsolva (a beleegyezés-ablak puszta bezárása ne számítson hozzájárulásnak), „Jogos érdek – alapértelmezés szerint engedélyezve" kikapcsolásra javasolva (óvatosabb megfelelőség), Consent Mode és IAB TCF 2. speciális funkció kikapcsolva hagyva (nem szükséges, mert nincs Google Analytics az oldalon).
- **`sutik.html`/`en/cookies.html` frissítve**: a jövő idejű "tervezzük"/"amint élesedik" megfogalmazás jelen időre cserélve, mivel az AdSense ténylegesen aktív.
- **Logó-buktató**: a webhely kiválasztásához a Google kötelezővé tette egy embléma feltöltését (bár a felületen elsőre opcionálisnak tűnt) — mivel a projektnek nem volt még saját logó-fájlja, egy egyszerű 512×512 PNG-t generáltunk a meglévő `jimp` függőséggel (Soft Calm accent szín + "QT" felirat), csak erre a célra, nem került a repóba.
- Ellenőrzés: mind a 18 fájlban pontosan egyszer szerepel a script tag (`grep`-pel igazolva); a felhasználó az AdSense felületén megerősítette a "Közzétéve" állapotot, HU+EN nyelvi lefedettséggel.

### ADR-018 – Nyomtatható PDF Készítő (TOOL-007): beágyazott Unicode betűtípus, nem szabványos PDF-font

- Állapot: **ELFOGADOTT**
- Dátum: 2026-08-26
- Döntés: az új "Nyomtatható PDF Készítő" tool (képek + Markdown-szöveg → egyetlen PDF, A4/A3) a `pdf-lib` szabványos `StandardFonts.Helvetica` helyett egy ténylegesen beágyazott Unicode betűtípust (Google Fonts "Noto Sans", `@pdf-lib/fontkit`-tel regisztrálva) használ a szövegrajzoláshoz.
- Indok: böngészős teszt közben kiderült, hogy a `StandardFonts.Helvetica` (és a többi 14 beépített PDF-font) csak WinAnsi-kódolást ismer, ami **nem tartalmazza a magyar hosszú ékezetes betűket** (ő, ű) — ezekre a `page.drawText()` kivétellel elszáll ("WinAnsi cannot encode..."). Mivel a tool elsődleges célközönsége magyar nyelvű szöveget írna bele, ez azonnal, minden gyakorlati használatnál hibát okozott volna.
- **`@pdf-lib/fontkit` csomagolási buktató**: a hivatalos ESM build (`dist/fontkit.es.js`) egy bundler-függő, nem felodható bare importot (`pako`) tartalmaz, ezért közvetlenül böngészőben, import map-pel nem tölthető be. Megoldás: az önálló UMD build (`dist/fontkit.umd.min.js`) betöltése sima `<script>`-ként (nem `type="module"`), ami a `window.fontkit` globálist állítja be; a modul-szkript onnan olvassa ki.
- **Tudatosan vállalt egyszerűsítés — nincs külön félkövér betűtípus**: a Google Fonts jelenlegi kiadásában a Noto Sans (és a legtöbb nagyobb család) csak variábilis fontként (`NotoSans[wdth,wght].ttf`), külön statikus Bold fájl nélkül érhető el a `google/fonts` GitHub-repóból. Emiatt a `**félkövér**` Markdown-jelölés jelenleg csak eltávolítja a csillagokat, de nem rajzol vizuálisan vastagabb szöveget — a helyes, teljes megoldás (pl. egy külön beszerzett statikus Bold TTF, vagy egy zip-csomagolt fontkészlet kibontása böngészőben) nagyobb munka lenne, nem indokolt egy első verzióban.
- **Két valódi buktató, amit a böngészős tesztelés tárt fel és amit javítottunk**: (1) kép után következő szövegelem ráíródott a kép oldalára, mert a szöveg-ág csak akkor nyitott új oldalt, ha *egyáltalán* nem volt még oldal (`if (!page) newPage()`), nem akkor, ha az *előző* elem kép volt — javítás: kép rajzolása után `page = null`, hogy a következő elem mindig friss oldalon induljon. (2) a záró biztonsági ellenőrzés (`if (!page) newPage()`) felesleges üres záró oldalt adott hozzá, valahányszor az utolsó elem kép volt (mert a fenti javítás miatt `page` ilyenkor `null`) — javítás: `pdfDoc.getPageCount() === 0` alapján dönt, nem a helyi `page` változó állapotából.
- Ellenőrzés: böngészőben (`javascript_exec`, `URL.createObjectURL` elfogás blob-típus szerint) igazolva — magyar ékezetes szöveg (ő/ű) hiba nélkül, A4 (595×842pt) és A3 (842×1191pt) pontos méret, kép+szöveg helyes sorrendben külön oldalakon (2 elem → pontosan 2 oldal, nem 1 vagy 3), hosszú szöveg helyes többoldalas tördelése (80 bekezdés → 5 oldal), sorrendezés (fel/le), eltávolítás és üres állapot mind helyesen működik.
